...
import static com.dremio.iceberg.testdata.Constants.SOURCE_BUCKET;
import static com.dremio.iceberg.testdata.Constants.SOURCE_REGION;
...
public class IcebergS3CopyData {
    ...
    public static void main(String[] args) {
        S3Configs s3Configs = new S3Configs(ACCESS_KEY, SECRET_KEY, SOURCE_BUCKET, SOURCE_REGION);
        //
        ...
        map.forEach((key, value) -> {
            ...
            for (int i = 219; i < 220; i++) {
                for (int j = 78; j < 79; j++) {
                    S3SyncCopy s3SyncCopy = new S3SyncCopy(s3Client, sourceFile,"testdata/optimize/mixed_size_dataset/ss_sold_date_sk="+ i +"/"+key+"_"+ j +".parquet" );
